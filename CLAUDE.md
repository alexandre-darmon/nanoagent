...
- **Tools with side effects stay fenced.** `write_file` is a real tool, not a skill. It writes only `.md` files inside `reports/` (only the base name of the requested path is kept), and it never overwrites an existing file — it adds a timestamp to the name instead. Keep those guarantees if you touch it.
